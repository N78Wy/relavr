...
- sender 既然固定依赖 WebSocket 信令和 WebRTC 网络状态监测，`app` manifest 必须同时声明 `android.permission.INTERNET` 与 `android.permission.ACCESS_NETWORK_STATE`；缺少前者会在 socket 建连阶段直接失败，缺少后者会在 WebRTC 初始化网络监测时触发 `SecurityException` 并导致 sender 崩溃。
