...
- sender 局域网发现固定镜像 `relavr-view` receiver 的 `_relavr-recv._tcp` mDNS 服务；host 以 NSD resolve 为准，TXT 字段固定解析 `name`、`ver`、`port`、`sessionId`、`auth`，其中 signaling 端口优先使用 TXT `port`，NSD resolve 端口只作为回退。
